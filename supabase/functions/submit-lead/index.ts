...
// Email templates
// ─────────────────────────────────────────
function tradespersonEmail(tradesperson: any, lead: any, tradeLabel: string): string {
  return `
  ...
  try {
    ...
    const [tradespersonSent, customerSent] = await Promise.all([
      sendEmail(
        ...
        tradespersonEmail(tradesperson, { customer_name: name, customer_email: email, customer_phone: phone, customer_postcode: postcode.toUpperCase(), message }, tradeRow.label)
      ),
      sendEmail(
        email,
